...
export default function setSelectedRaster() {
  ...
  const setRasterLayer = async function () {
    let { vectData, sldRequest, rasterName } = null;
    let rasterYear = store.getYearSelected;

    store.getselectedSubRegion
      ? (vectData = {
          name: store.getselectedSubRegion,
          adminLevel: 1,
          country: store.getselectedCountry,
        })
      : store.getselectedCountry
      ? (vectData = {
          name: store.getselectedCountry,
          adminLevel: 0,
          country: null,
        })
